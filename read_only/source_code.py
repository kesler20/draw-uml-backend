...
class SourceCode:
    def __init__(self, response_code_read_path: str) -> None:
        self.source: ClassRepresentation = self.read_and_clean_source(
            response_code_read_path)
    ...
    def read_and_clean_source(self, response_code_read_path: str) -> ClassRepresentation:
        """read one class at a time, then pop from the array
        do the required cleanups and preprocessing of the class"""

        # remove the end of line delimeter from every line of the response_code file (json file)
        source: ClassRepresentation = [line.replace('\n', "") for line in File(
            response_code_read_path).readlines()][-1]
