...
class TypeChecker(BaseReader):
    ...
    types_file: str
    __built_in_types: List[str] = field(
        init=False,
    ...
    __mutable_types: List[str] = field(
        init=False, default_factory=lambda: ["List", "list", "Dict", "dict"]
    )
    ...
    def novel_types(self) -> Set[str]:
        ...
        novel_types = list(
            filter(lambda item: item not in [*self.built_in_types, *self.typing_types], clean_types)
        )
    ...
    def change_source_to_typing(self):
        ...
        source["properties"] = [
            [name[0], self.convert_builtin_to_typing(name[1])] for name in source["properties"]
        ]
        source["fields"] = [
            [name[0], self.convert_builtin_to_typing(name[1])] for name in source["fields"]
        ]
        ...
        for callable in source["methods"]:
            try:
                callable["params"] = [
                    [param[0], self.convert_builtin_to_typing(param[1])]
                    for param in callable["params"]
                ]
                callable["return_type"] = self.convert_builtin_to_typing(callable["return_type"])
